...
This also answers the continuum-reductionist (the chariot, *na ca so na ca añño*). Their move fixes *whose fruit?* by convention over a causal continuum, exactly as *which chariot?* is fixed over parts. But that treats ownership as a *thing*-designation — a mine-*being* had over a base — when karma's ownership is an *act*-designation: mine-*making*, the weld enacted at act-time (MMK 8). A thing-designation can be fixed by convention over a static base; an act-designation cannot, because the "whose" is *made*, not *had* — there is no static base over which convention could fix it, only a weld that must be enacted. So the verdict is neither merely verbal (the I-making is a real act-time event, not a relabelling of a state the reductionist already grants) nor a smuggled soul (the weld is fully empty, MMK 8, no base behind it). It is malformed in the precise sense that a *thing*-fixing convention is offered where only *act*-enactment can supply the index.
